...
#                  if math.cos(r) > 0:       if math.cos(r + k):
#                      count = 1                count = count + 1
#                  return count          return count
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean? We ignore the loop variable.
#
...
#        and one is WRONG.  Which is the WRONG one? The second one updates x  my diameter and by a multiple of k.
#
#              x = starting_point.x
#              for k in range(n):
